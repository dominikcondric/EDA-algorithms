from numpy import *
import sys, random
from algorithms.boa_utils.bic_metric import *
...
class BayesianNetwork:
    ...
    def learn_univariate(self, node):
        ...
        if round(sum(list(self.prior_table[node].values()))-1,7) != 0:
            for i in [0, 1]:
    ...
    def learn_conditional(self, node):
        if (node, 0) in self.cond_table.keys():
    ...
    def calc_pi(self, child):
        ...
        if len(parents) == 0:
            ...
            pi = map(lambda x:[x], [0, 1])
        else:
            pi = map(lambda x:[x], [0, 1])
            for p in parents[1:]:
                pi = map(lambda x: list(x),itertools.product(pi,[0, 1]))
